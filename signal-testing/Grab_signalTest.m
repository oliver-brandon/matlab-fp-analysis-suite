...
% ind1 = find(time>=35,1);
% ind2 = find(time>=37,1);
% ISOS_raw_split1 = ISOS_raw(1,1:ind1);
% ISOS_raw_split2 = ISOS_raw(1,ind2:end);
% ISOS_raw = [ISOS_raw_split1 ISOS_raw_split2];
% 
% Grab_raw_split1 = Grab_raw(1,1:ind1);
% Grab_raw_split2 = Grab_raw(1,ind2:end);
% Grab_raw = [Grab_raw_split1 Grab_raw_split2];
% 
% time = time(1:length(Grab_raw));




% % Perform ratiometric correction
% ratio = Grab_raw ./ ISOS_raw; % calculate ratio of raw signal to isosbestic signal
% ratio_smoothed = smoothdata(ratio); % smooth the ratio signal to remove noise
% Grab_raw = ratio_smoothed .* ISOS_raw; % multiply smoothed ratio signal by isosbestic signal
...
ind1 = find(time > x,1);
ind2 = find(time > y,1);
Grab_snip = Grab_dFF_z(ind1:ind2);
