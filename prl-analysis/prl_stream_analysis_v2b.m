...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
myDir = uigetdir(...
    'E:\Google Drive\prl\PRL_GRABDA\goodSignals','Choose the .mat files you want to analyze.'...
    ); %gets directory%
if myDir == 0
...
for i = 1:numFiles
    ...
    if isempty(cRew_cueBase)
        ...
        auc_cRew_cueBase(i,1) = calculateAUC(mean(cRew_cueBase(:,aucSt:aucEn),1,'omitnan'),ts1);
    ...
    if isempty(cNoRew_cueBase)
        ...
        auc_cNoRew_cueBase(i,1) = calculateAUC(mean(cNoRew_cueBase(:,aucSt:aucEn),1,'omitnan'),ts1);
    ...
    if isempty(iRew_cueBase)
        ...
        auc_iRew_cueBase(i,1) = calculateAUC(mean(iRew_cueBase(:,aucSt:aucEn),1,'omitnan'),ts1);
    ...
    if isempty(iNoRew_cueBase)
        ...
        auc_iNoRew_cueBase(i,1) = calculateAUC(mean(iNoRew_cueBase(:,aucSt:aucEn),1,'omitnan'),ts1);
    ...
    for k = 1:numel(epocList)
        ...
        auc_cue_cueBase(i,1) = calculateAUC(mean(cue_cueBase(:,aucSt:aucEn),1,'omitnan'),ts1);
