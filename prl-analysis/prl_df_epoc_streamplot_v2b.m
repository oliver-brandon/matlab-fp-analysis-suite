...
fprintf("prl_df_epoc_streamplot Version: %s\n",VERSION)
tic
if myDir == 0
...
for batch = 1:numFiles
    ...
    for i = 1:length(epoc_streamA)
        ...
        plot(ts1, mean_dataA, 'b-', 'LineWidth', 3, 'DisplayName',['Mean ',stream_A]);
        hold on
        plot(ts1, mean_dataC, 'r-', 'LineWidth', 3, 'DisplayName',['Mean ',stream_C]);
        ...
        h = fill(XX, YY_A, 'r', 'DisplayName', ['SEM ',stream_A]);
        g = fill(XX, YY_C, 'b', 'DisplayName',['SEM ',stream_C]);
