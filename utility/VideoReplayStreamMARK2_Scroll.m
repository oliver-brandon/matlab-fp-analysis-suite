...
BLOCKPATH = '/Users/brandon/personal-drive/collaborations/yamanaka-cat-saliva/2_FPSF1_4_FPSF1';
data = TDTbin2mat(BLOCKPATH, 'TYPE',{'epocs', 'scalars', 'streams'}); %,'T1',1500,'T2',1600,
savePATH = '/Users/brandon/Desktop';
%% Importing the Data
...
ISOS = 'x405A';     % name of the 405A store DLS
SIGNAL = 'x465A';       % name of the 465A store DLS
...
ROLLING = 20;          % rolling window, in seconds (use -1 for none)
...
data.streams.(SIGNAL).data = arrayfun(@(i)...
    mean(data.streams.(SIGNAL).data(i:i+N-1)),...
    1:N:length(data.streams.(SIGNAL).data)-N+1);
...
fs = data.streams.(SIGNAL).fs/N; %DLS% /N as recall we downsampled by N = 10 earlier
...
meanSignal1 = mean(ISOS);
stdSignal1 = std(double(ISOS))/sqrt(size(ISOS,1));
dcSignal1 = mean(meanSignal1);
medianSignal1 = median(ISOS); % median of fluorescence signal for dF/F calc

% Create mean signal, standard error of signal, and DC offset of DLS_DA signal
meanSignal1 = mean(SIGNAL);
stdSignal1 = std(double(SIGNAL))/sqrt(size(SIGNAL,1));
dcSignal1 = mean(meanSignal1);
medianSignal1 = median(SIGNAL); % median of fluorescence signal for dF/F calc
...
recording_ts = data.epocs.(VID_STORE).onset(k);
 stream_ind = round(recording_ts * data.streams.(SIGNAL).fs);
    if ct == 1
...
%%
% Read video file.
vvv = dir([BLOCKPATH filesep '*' VID_STORE '.m4v']);
if isempty(vvv)
    error('No video file found matching the pattern %s in the directory %s', ...
        [BLOCKPATH filesep '*' VID_STORE '.m4v'], BLOCKPATH);
end
vid_filename = fullfile(vvv.folder, vvv.name);
fprintf('Reading file: %s\n', vid_filename);
try
    myvideo = VideoReader(vid_filename);
catch ME
    error('Error reading video file: %s\n%s', vid_filename, ME.message);
end
%%
% Get data specs.
...
max_x = max(size(data.streams.(SIGNAL).data));
...
for k = START_FRAME:END_FRAME
    ...
    stream_ind = (round(recording_ts * data.streams.(SIGNAL).fs)/N);
    if ct == 1
        start_ind = stream_ind;
        end_ind = (round(data.epocs.(VID_STORE).onset(END_FRAME) * data.streams.(SIGNAL).fs)/N);
    end
    
    if ROLLING > 0
%         stream_ind = round(recording_ts * data.streams.(DLS_DA).fs);
        max_ts = data.epocs.(VID_STORE).onset(k);
        start_ind = (max(round(max(recording_ts - ROLLING, 0) * data.streams.(SIGNAL).fs), 1)/N);
