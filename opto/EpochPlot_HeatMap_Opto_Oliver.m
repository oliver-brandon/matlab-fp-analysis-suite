...
BLOCKPATH = '/Users/brandon/personal-drive/optomouse-prime/opto-reversal/cue-stim/mats-cuestim-VTA/Ipsilateral/1775_Rev3CueStimBody_tdTom.mat'; % path to TDT data tank (folder containing TDT data)
...
ROI = channel1ROI; % Can change depending on ROI (used for figure title)
% remove any "/" from REF_EPOC
REF_EPOC2 = strrep(REF_EPOC2,'/','');
TITLE = strcat(ID,{'-'},task,{'-'},ROI,{'-'},REF_EPOC2);
% Optionally remove artifacts. If any waveform is above ARTIFACT level, or
% below -ARTIFACT level, remove it from the data set.
art1 = ~cellfun('isempty', cellfun(@(x) x(x>ARTIFACT405), ...
    data.streams.(STREAM_STORE1).filtered, 'UniformOutput',false));
...
for k = 1:height(zall2)
    zallSmooth2(k,:) = smoothdata(zall2(k,:),'movmean',smoothFactor);
end
...
meanZall_smooth2 = mean(zallSmooth2);
...
if setBaseline == 1
    ...
    for base = 1:height(zall2)
        if zall2(base,idx) < 0
            val = zall2(base,idx);
            diff = 0 - val;
            zall2(base,:) = zall2(base,:) + abs(diff);
        elseif zall2(base,idx) > 0
            val = zall2(base,idx);
            diff = 0 - val;
            zall2(base,:) = zall2(base,:) - abs(diff);
        end
...
% Standard error of the z-score
meanZall2 = mean(zallSmooth2);
zerror = std(zallSmooth2)/sqrt(size(zallSmooth2,1));
...
% correctly
XX = [ts2, fliplr(ts2)];
YY = [mean(zallSmooth2)-zerror, fliplr(mean(zallSmooth2)+zerror)];

subplot(3,1,3)
plot(ts2, mean(zallSmooth2), 'color',[0.8500, 0.3250, 0.0980], 'LineWidth', 3); hold on;
line([0 0], [min(YY*1.5), max(YY*1.5)], 'Color', [.7 .7 .7], 'LineWidth', 2)
...
if saveFig == 1
    if figType == 1
        % Save figure 4 to saveFigPath
        file_name1_pdf = char(strcat(figSavePath,TITLE,'.pdf'));
        orient(f4,'landscape');
        print(f4,file_name1_pdf,'-dpdf','-vector','-bestfit','');
        
    elseif figType == 2
        file_name1_png = char(strcat(figSavePath,TITLE,'.png'));
        orient(f4,'landscape');
        print(f4,file_name1_png,'-dpng','-r300');
    end
